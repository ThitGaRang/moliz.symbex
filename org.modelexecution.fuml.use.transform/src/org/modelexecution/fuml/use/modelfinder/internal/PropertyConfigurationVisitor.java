...
import org.tzi.kodkod.model.config.impl.ClassQueryConfigurator;
...
public class PropertyConfigurationVisitor extends SimpleVisitor {
	...
	public PropertyConfigurationVisitor() {
		init();
	}

	public PropertyConfigurationVisitor(String file)
			throws ConfigurationException {
		setConfig(new PropertiesConfiguration(file));
		init();
	}
	...
	public void visitClass(IClass clazz) {
		...
		for (String element : readSingleElements(name)) {
			...
			if (!(element.equals(TypeConstants.UNDEFINED) || element
					.equals(TypeConstants.UNDEFINED_SET))) {
	...
	public void visitAttribute(IAttribute attribute) {
		...
		domainValues.addAll(readAttributeValues(attribute.type(), className
				+ "_" + attribute.name(), typeSpecificValues));

		readSpecificAttributeValues(attribute, owner, specificValues,
				typeSpecificValues);

		setAttributeConfigurator(attribute, className, specificValues,
				domainValues);
	...
	public void visitAssociation(IAssociation association) {
		setAssociationConfigurator(association,
				readComplexElements(association));
	}
			...
			if (status.equals("negate")) {
				...
				if (!status.equals("active")) {
					warning(invariant.name() + ": "
							+ LogMessages.invariantConfigWarning(status));
				}
	...
	public void visitRealType(RealType realType) {
		...
		double step = config.getDouble(
				realType.name() + PropertyEntry.realStep, 0.5);
	...
	public void visitConfigurableType(ConfigurableType type) {
		...
		if (type.isInteger()) {
			...
			if (type.isString()) {
				min = readSize(type.name() + PropertyEntry.stringValuesMin, 0,
						false);
				max = readSize(type.name() + PropertyEntry.stringValuesMax, 1,
						false);
			} else {
				if (type.isReal()) {
					min = readSize(type.name() + PropertyEntry.realValueMin, 0,
							true);
					max = readSize(type.name() + PropertyEntry.realValueMax, 1,
							true);
				}
			}
		}
	...
	protected void setModelConfigurator(IModel model) {
		...
		if (model.getConfigurator() instanceof ModelConfigurator) {
			...
			configurator = new ModelConfigurator(model,
					new HashMap<String, IInvariant>());
			model.setConfigurator(configurator);
		}

		String cyclefreenessState = config.getString(
				PropertyEntry.aggregationcyclefreeness, "off");
		if (cyclefreenessState.equals("on")) {
			...
			if (cyclefreenessState.equals("off")) {
				...
				configurator
						.setAggregationCycleFreeness(DefaultConfigurationValues.aggregationcyclefreeness);
			}
		}

		String forbiddensharingState = config.getString(
				PropertyEntry.forbiddensharing, "on");
		if (forbiddensharingState.equals("on")) {
			...
			if (forbiddensharingState.equals("off")) {
				...
				configurator
						.setForbiddensharing(DefaultConfigurationValues.forbiddensharing);
			}
		}
	}
	 ...
	 * @return
	 */
	protected ClassConfigurator setClassConfigurator(IClass clazz,
			List<String[]> values) {
		...
		configurator.setLimits(min,
				readSize(clazz.name() + PropertyEntry.objMax, min, false));
	 ...
	 * @param domainValues
	 */
	protected void setAttributeConfigurator(IAttribute attribute,
			String className, List<String[]> specificValues,
		...
		configurator.setLimits(
				readSize(searchName + PropertyEntry.attributeDefValuesMin, 0,
						false),
				readSize(searchName + PropertyEntry.attributeDefValuesMax, -1,
						false));
		configurator.setCollectionSize(
				readSize(searchName + PropertyEntry.attributeColSizeMin, 0,
						false),
				readSize(searchName + PropertyEntry.attributeColSizeMax, -1,
						false));
	 ...
	 * @param values
	 */
	protected void setAssociationConfigurator(IAssociation association,
			List<String[]> values) {
		AssociationConfigurator configurator = new AssociationConfigurator();
		configurator.setSpecificValues(values);
		configurator
				.setLimits(
						readSize(association.name() + PropertyEntry.linksMin,
								0, false),
						readSize(association.name() + PropertyEntry.linksMax,
								-1, false));
	...
	private int readSize(String name, int defaultValue, boolean allowNegative) {
		...
		try {
			...
			warning(name + ": "
					+ LogMessages.sizeConfigWarning(name, defaultValue));
			limit = defaultValue;
		} catch (Exception e) {
			limit = defaultValue;
		}
	...
	private String[] readSingleElements(String name) {
		...
		if (elements.length == 0) {
			...
			if (elements.length == 1
					&& elements[0].equals(TypeConstants.UNDEFINED_SET)) {
				return new String[] { TypeConstants.UNDEFINED_SET };
			}
		}

		checkSetSyntax(elements[0], elements[elements.length - 1], name,
				"Set{a,b,c}");
			...
			for (String split : splits) {
				...
				for (int i = 0; i < singleElements.length; i++) {
					...
					if (i == 0 && association.associationClass() != null) {
						...
						className = association.associationEnds().get(j)
	...
	private void checkComplexElement(String name, String input, String element,
			String className) {
		if (!classSpecificValues.get(className).contains(element)) {
			...
			if (element.startsWith(className.toLowerCase())) {
				...
				try {
					...
					if (!(nbr > classSpecificValues.get(className).size() && nbr <= classMinObjects
							.get(className))) {
	...
	private void complexElementError(String name, String input, String element,
			String className) {
	...
	private void checkSetSyntax(String start, String end, String name,
			String syntax) {
	 ...
	 */
	private void addTypeSpecificValues(Type type,
			Collection<String[]> typeSpecificValues) {
		if (type.isSet()) {
	...
	private List<String[]> getTypeSpecificValues(Type type) {
		if (typeSpecificValues.get(type) == null) {
			typeSpecificValues.put((ConfigurableType) type,
	 ...
	 * @param typeSpecificValues
	 */
	private void readSpecificAttributeValues(IAttribute attribute,
			IClass owner, List<String[]> specificValues,
		...
		for (IClass clazz : getClassSpecificValues(owner).keySet()) {
			for (String object : specificClassValues.get(clazz)) {
				...
				if (attribute.type().isSet()) {
					Set<String> values = readAttributeValues(attribute.type(),
							searchName, typeSpecificValues);
	 ...
	 * @return
	 */
	private Set<String> readAttributeValues(Type type, String readName,
			Set<String[]> typeSpecificValues) {
	...
	private String adjustElement(Type type, String element) {
		element = element.trim();
		if (element.equals(TypeConstants.UNDEFINED)
				|| element.equals(TypeConstants.UNDEFINED_SET)) {
			return element;
		}
