...
class FumlValues2UseValuesTest extends TestCase {
  ...
  @Test def testTransformingUniversityManagementSystem {
    ...
    assertEquals(fUmlValues.size, useValues.size)

    assertTrue(fUmlValues.forall { value =>
      value eq fUmlValues2UseValues.fuml(fUmlValues2UseValues.use(value))
    })

    fUmlValues.foreach { value =>
      assertEqualsValue(value, fUmlValues2UseValues.use(value))
    }
  }
